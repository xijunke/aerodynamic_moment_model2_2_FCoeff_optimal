...
% tic
% x =[1,1,1,1];  % obj_function=40.1316;
% x =[1,0.5,0.5716,0.0241];  % fval =39.5202; % 混合GA——针对两只翅膀的气动力系数
% x =[1,0.5,0.571,0.01];    % fval =39.5127;    % 模式搜索
% x =[1.0000,0.5000,0.3358,0.5000];       % fval=36.5981; 
k_C_N=x(1);
k_C_R=x(2); % 其取值C_R=1.55; k_C_R*C_R可以具有一定的任意性
k_add=x(3); % 虚质量系数取0.5是可以考虑的
k_inert=x(4); % 惯性质量可有可无=0.0241
...
% force_science=load('force_science.csv');  % 载入数据
...
% plot(t/T,2*F_vertical/W,'r-',t/T,2*F_horiz_Y/W,'b-','LineWidth',2)  
...
% legend('F_{z,exp}','F_{x,exp}','F_{vertical,Z,cal}','F_{horiz,Y,cal}')
...
stdev_v=std((2*F_vertical-Fz_norm),0,1);   % 按照S1(0)求解各列(1)元素的标准方差
stdev_h=std((2*F_horiz_Y-Fx_norm),0,1);   % 按照S1(0)求解各列(1)元素的标准方差
obj_function=stdev_v+stdev_h+penaltyfun;
% save('obj_function.txt', 'obj_function', '-ASCII')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% toc